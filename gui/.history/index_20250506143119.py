...
def open_images():
    ...
    # Build output path with today's date
    today_str = datetime.date.today().strftime("%Y-%m-%d")
    script_dir = os.path.dirname(os.path.abspath(__file__))
    output_path = os.path.join(script_dir, f"output_{today_str}.pdf")
    ...
    # Create PDF and add each image
    c = canvas.Canvas(output_path)
    for image_path in image_files:
        print(f"🔄 Processing: {image_path}")
        add_image_to_pdf_canvas(image_path, c)
    c.save()
    print(f"✅ Multi-page PDF saved to: {output_path}")
