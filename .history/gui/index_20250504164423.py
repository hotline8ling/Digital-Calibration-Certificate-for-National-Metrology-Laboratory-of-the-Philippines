...
def open_format():
    ...
    try:
        # First verify if the file is a valid PDF
        try:
            # Just test-open the PDF to validate it
            with pdfplumber.open(pdf_path) as test_pdf:
                pass
        except Exception as pdf_error:
            if "No /Root object" in str(pdf_error):
                messagebox.showerror("Invalid PDF", 
                    "The selected file appears to be invalid or corrupted. Please select a valid PDF file.")
            elif "password" in str(pdf_error).lower():
                messagebox.showerror("Protected PDF", 
                    "The selected PDF is password-protected. Please use an unprotected PDF.")
            else:
                messagebox.showerror("PDF Error", f"Error opening PDF: {str(pdf_error)}")
            return
 
        # If we get here, the PDF is valid, proceed with extraction
        ...
        if output:
            ...
            app.destroy()
            subprocess.Popen(["python", "pdfToxml.py", output])
